...
const isStartDate = (dates: Reservation[], currentIndex: number): boolean => {
...
const isEndDate = (dates: Reservation[], currentIndex: number): boolean => {
  if (
...
const CalendarComponent = () => {
  ...
  useEffect(() => {
    fetch(`/api/rentlio?dateFrom=${currentDates[0]}&dateTo=${currentDates[1]}`)
      .then((result) =>
        result.json().then((data: Reservation[]) => {
